...
def tracked_files() -> set[str]:
    # Full, resolved path (not the bare "git" string) so ruff's S607 (partial executable path)
    # is actually satisfied, not just silenced -- a PATH-relative subprocess call can pick up an
    # unexpected binary if PATH is ever attacker-influenced; resolving via shutil.which() first
    # pins exactly which git this runs before subprocess.run ever sees it.
    git_path = shutil.which("git")
    if git_path is None:
        print("git not found on PATH -- cannot verify tracked files")
        sys.exit(1)
    out = subprocess.run(  # noqa: S603 -- fixed argv (no shell, no user input), git_path resolved above
        [git_path, "ls-files"], capture_output=True, text=True, check=True
    )
